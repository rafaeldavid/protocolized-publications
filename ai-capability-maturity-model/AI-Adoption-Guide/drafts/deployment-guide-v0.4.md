...
> ### Note on this chapter *(placeholder — v0.4)*
>
...
Across organizations that have moved up the maturity model, six patterns recur. Across organizations that stalled or produced consequential failures, six failure modes recur. Both lists are useful as a checklist before pilot decisions and as a debrief framework after them.
...
**Visibility rituals.** A recurring, low-judgment forum where members of a function show what they have built, what they have tried, and what has failed. The biweekly show-and-tell described in Chapter 5, Case Study 1 is the cheapest viable instance. Visibility is the precondition for harvest. Without it, kit work stays personal and the failure modes that follow from disorientation compound silently.
...
**Output-scope governance, not access-scope.** The protocol that matters is the one that governs what AI produces, who reviews it, and where the agent's permissions end before a consequential decision is made. An access policy is necessary but does not produce reliability on its own.
...
**Capability erosion.** AI replaces work that developed the skill to verify it. The short-horizon version is the turnkey replacement of dual production-and-QA roles without reliable edge-case management. The long-horizon version is the loss of the underlying competency itself. Junior associates who would have learned contract review or financial analysis through repetition no longer do, and the function gradually loses the ability to verify the AI's output because no one in it has the underlying skill anymore.
